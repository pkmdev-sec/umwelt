...
PROFILES_DIR="$UMWELT_DIR/profiles"
...
test_dev_profile_header() {
  ...
  "$UMWELT_BIN" profile dev 2>&1 | grep -q "DEVELOPMENT\|DEV" || true
}
...
test_review_profile_runs() {
  "$UMWELT_BIN" profile review &>/dev/null || true
}

test_review_profile_with_base_branch() {
  "$UMWELT_BIN" profile review main &>/dev/null || true
}

test_review_profile_with_develop_branch() {
  "$UMWELT_BIN" profile review develop &>/dev/null || true
}

test_review_profile_loads_git_context() {
  # review profile should include git-context
  "$UMWELT_BIN" profile review 2>&1 | grep -q "GIT\|diff\|REVIEW" || true
}
...
test_review_profile_header() {
  ...
  "$UMWELT_BIN" profile review 2>&1 | grep -q "REVIEW\|CODE REVIEW" || true
}
...
test_all_profiles_have_umwelt_dir() {
  for profile in "$PROFILES_DIR"/*.sh; do
    [ -f "$profile" ] || continue
    # Each profile should set UMWELT_DIR
    grep -q "UMWELT_DIR=" "$profile" || true
  done
}
...
test_profiles_via_umwelt_command() {
  # Test all profiles via the umwelt command
  for profile_file in "$PROFILES_DIR"/*.sh; do
    [ -f "$profile_file" ] || continue
    profile_name=$(basename "$profile_file" .sh)
    "$UMWELT_BIN" profile "$profile_name" &>/dev/null || true
  done
}
...
test_profile_invalid_name() {
  ...
  local exit_code=0
  "$UMWELT_BIN" profile nonexistent-profile-xyz &>/dev/null || exit_code=$?
  [ "$exit_code" -ne 0 ]
}
...
test_profile_no_name() {
  ...
  local exit_code=0
  "$UMWELT_BIN" profile &>/dev/null || exit_code=$?
  [ "$exit_code" -ne 0 ]
}
