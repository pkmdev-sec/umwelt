#!/usr/bin/env bash
# umwelt: Cache warming — runs on SessionStart
...
mkdir -p "${UMWELT_CACHE_DIR:-$HOME/.claude/.bang-cache}"
...
for loader in "${LOADERS[@]}"; do
  LOADER_PATH="$UMWELT_DIR/loaders/${loader}.sh"
...
TS_FILE="${UMWELT_CACHE_DIR:-$HOME/.claude/.bang-cache}/git-context-last-call"
date +%s > "$TS_FILE"
